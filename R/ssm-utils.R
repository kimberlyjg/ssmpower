...
#'
#' Calculates an asymptotic confidence interval for amplitude.
#' The lower bound is truncated at 0 since amplitude cannot be negative.
#'
...
ssm_ci_amplitude <- function(amplitude, n, conf_level = 0.95, k = 0.41) {
 ...
 lower <- max(0, amplitude - z * se)  # Truncate at 0
...
#'
#' Calculates an asymptotic confidence interval for elevation.
#'
...
#' Effect Size Label
#'
#' Interprets an SSM effect size based on benchmarks from Zimmermann & Wright
#' (2017). Returns a categorical label describing effect magnitude.
#'
#' @param effect Effect size value (amplitude or elevation).
#' @param type Either "amplitude" or "elevation".
#'
#' @return Character string: "< Small", "Small", "Small-Medium", "Medium",
#'   "Medium-Large", or "Large".
#'
#' @details
#' Benchmarks are derived from 433 published SSM analyses:
#'
#' **Amplitude:**
...
ssm_effect_label <- function(effect, type = c("amplitude", "elevation")) {
 ...
 if (type == "amplitude") {
   # Benchmarks: Small = 0.10, Medium = 0.16, Large = 0.23
   ...
   # Benchmarks: Small = 0.02, Medium = 0.11, Large = 0.27
...
#' Sample Size Quick Reference Guide
#'
#' Displays a formatted table of sample size requirements for common
#' effect sizes at 80% power (or other specified level).
#'
#' @param power Target power level. Default is 0.80.
#'
#' @return Invisibly returns a data frame with the computed values.
#'
#' @details
#' The guide shows required sample sizes for:
#' - Single-sample amplitude and elevation tests
#' - Two-group amplitude comparisons
#'
#' Effect size benchmarks are from Zimmermann & Wright (2017).
#'
#' @examples
...
#'
#' Creates a table of power values across combinations of effect sizes
#' and sample sizes.
#'
#' @param effects Numeric vector of effect sizes to evaluate.
#' @param ns Numeric vector of sample sizes to evaluate.
#' @param type Either "amplitude" or "elevation".
#' @param alpha Significance level. Default is 0.05.
#'
#' @return A data frame of class `ssm_power_table` with power values.
#'   Rows represent effect sizes, columns represent sample sizes.
#'
#' @examples
#' ssm_power_table(
