...
# =============================================================================
# Based on:
#   - Zimmermann & Wright (2017). Assessment, 24, 3-23.
#   - Gurtman (1992). JPSP, 63, 105-118.
#   - Cohen (1988, 1992). Statistical Power Analysis.
# =============================================================================
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        conditionalPanel(
          ...
          numericInput("power_n1", "Group 1 Size (n₁)", value = 100, min = 10, max = 5000),
          numericInput("power_n2", "Group 2 Size (n₂)", value = 100, min = 10, max = 5000)
        ),
        
        numericInput("power_alpha", "Alpha (α)", value = 0.05, min = 0.001, max = 0.20, step = 0.01),
        ...
        p(class = "text-muted small",
          ...
          br(),
          strong("Amplitude:"), " Small=0.10, Med=0.16, Large=0.23",
          br(),
  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        numericInput("ss_alpha", "Alpha (α)", value = 0.05, min = 0.001, max = 0.20, step = 0.01),
        ...
        conditionalPanel(
          ...
          numericInput("ss_ratio", "Allocation Ratio (n₂/n₁)", value = 1, min = 0.5, max = 4, step = 0.5)
        ),
  ...
  nav_panel(
    ...
    layout_sidebar(
      sidebar = sidebar(
        ...
        numericInput("table_alpha", "Alpha (α)", value = 0.05, min = 0.001, max = 0.20),
      ...
      card(
        ...
        card_body(
          ...
          p(class = "text-muted mt-3",
            em("Where: a = expected effect, k = scaling constant, n = sample size"))
        )
      ),
  ...
  nav_panel(
    ...
    card(
      ...
      card_body(
        ...
        h4("Citation"),
        p("If you use this toolkit, please cite:"),
        tags$blockquote(class = "blockquote",
          "Zimmermann, J., & Wright, A. G. C. (2017). Beyond description in interpersonal construct validation: Methodological advances in the circumplex Structural Summary Approach. ", 
...
server <- function(input, output, session) {
  ...
  # Preset buttons for sample size calculator
  ...
  output$power_results <- renderUI({
    ...
    if (input$power_design == "single") {
      ...
      if (input$power_type == "amplitude") {
        result <- ssm_power_amplitude(input$power_effect, input$power_n, 
                                       input$power_alpha, one_sided = input$power_onesided)
      } else {
        result <- ssm_power_elevation(input$power_effect, input$power_n, 
                                       input$power_alpha, one_sided = input$power_onesided)
      }
      ...
      n_display <- paste0("n₁ = ", result$n1, ", n₂ = ", result$n2, " (Total = ", result$n1 + result$n2, ")")
    }
    ...
    tagList(
      div(class = "row",
        div(class = "col-md-6",
          h2(class = paste("mb-0", power_class), paste0(power_pct, "%")),
          p(class = "text-muted", "Statistical Power")
        ),
        div(class = "col-md-6",
          h4(effect_label),
          p(class = "text-muted", "Effect Size Category")
        )
      ),
      hr(),
      tags$table(class = "table table-sm",
        tags$tbody(
          ...
          tags$tr(tags$td("Alpha (α):"), tags$td(result$alpha)),
      ...
      if (result$power < 0.80) {
        div(class = "alert alert-warning mt-3",
          ...
          " Power is below the conventional 80% threshold. Consider increasing sample size or expecting a larger effect."
        )
      }
    )
  })
  ...
  output$power_curve <- renderPlot({
    ...
    if (input$power_design == "single") {
      if (input$power_type == "amplitude") {
        powers <- sapply(ns, function(n) ssm_power_amplitude(input$power_effect, n, 
                                                              input$power_alpha, one_sided = input$power_onesided)$power)
      } else {
        powers <- sapply(ns, function(n) ssm_power_elevation(input$power_effect, n, 
                                                              input$power_alpha, one_sided = input$power_onesided)$power)
      }
    ...
    current_power <- if (input$power_design == "single") {
      if (input$power_type == "amplitude") {
        ssm_power_amplitude(input$power_effect, current_n, input$power_alpha, one_sided = input$power_onesided)$power
      } else {
        ssm_power_elevation(input$power_effect, current_n, input$power_alpha, one_sided = input$power_onesided)$power
      }
    } else {
      if (input$power_type == "amplitude") {
        ssm_power_amplitude_diff(input$power_effect, input$power_n1, input$power_n2, input$power_alpha)$power
      } else {
        ssm_power_elevation_diff(input$power_effect, input$power_n1, input$power_n2, input$power_alpha)$power
      }
    }
    ...
    ggplot(df, aes(x = n, y = power)) +
      ...
      labs(x = "Sample Size (N)", y = "Statistical Power",
           title = paste("Power Curve for", tools::toTitleCase(input$power_type)),
           subtitle = paste("Effect =", input$power_effect)) +
  ...
  output$ss_results <- renderUI({
    ...
    if (input$ss_design == "single") {
      if (input$ss_type == "amplitude") {
        result <- ssm_sample_size_amplitude(input$ss_effect, input$ss_power, 
                                             input$ss_alpha, one_sided = input$ss_onesided)
      } else {
        result <- ssm_sample_size_elevation(input$ss_effect, input$ss_power, 
                                             input$ss_alpha, one_sided = input$ss_onesided)
      }
      n_display <- paste("N =", result$n)
      test_type <- if (result$one_sided) "One-sided" else "Two-sided"
      ...
      if (input$ss_type == "amplitude") {
        result <- ssm_sample_size_amplitude_diff(input$ss_effect, input$ss_power, 
                                                  input$ss_alpha, ratio = input$ss_ratio)
      } else {
        result <- ssm_sample_size_elevation_diff(input$ss_effect, input$ss_power, 
                                                  input$ss_alpha, ratio = input$ss_ratio)
      }
      n_display <- paste0("n₁ = ", result$n1, ", n₂ = ", result$n2)
      test_type <- "Two-sided"
    }
      ...
      div(class = "row",
        div(class = "col-md-6",
          h2(class = "mb-0 text-primary", 
             if (input$ss_design == "single") result$n else result$n_total),
          p(class = "text-muted", if (input$ss_design == "single") "Required N" else "Total N Required")
        ),
        div(class = "col-md-6",
          h4(paste0(round(result$achieved_power * 100, 1), "%")),
          p(class = "text-muted", "Achieved Power")
        )
      ),
      hr(),
      tags$table(class = "table table-sm",
    ...
    if (input$ss_design == "single") {
      if (input$ss_type == "amplitude") {
        sample_sizes <- sapply(effects, function(e) 
          ssm_sample_size_amplitude(e, input$ss_power, input$ss_alpha, one_sided = input$ss_onesided)$n)
      } else {
        sample_sizes <- sapply(effects, function(e) 
          ssm_sample_size_elevation(e, input$ss_power, input$ss_alpha, one_sided = input$ss_onesided)$n)
      }
    } else {
      if (input$ss_type == "amplitude") {
        sample_sizes <- sapply(effects, function(e) 
          ssm_sample_size_amplitude_diff(e, input$ss_power, input$ss_alpha, ratio = input$ss_ratio)$n_total)
      } else {
        sample_sizes <- sapply(effects, function(e) 
          ssm_sample_size_elevation_diff(e, input$ss_power, input$ss_alpha, ratio = input$ss_ratio)$n_total)
      }
    }
    
    df <- data.frame(effect = effects, n = sample_sizes)
    
    benchmarks <- if (input$ss_type == "amplitude") {
      data.frame(effect = c(0.10, 0.16, 0.23), label = c("Small", "Medium", "Large"))
    } else {
      data.frame(effect = c(0.02, 0.11, 0.27), label = c("Small", "Medium", "Large"))
    }
    
    # Get the y value for the current effect
    current_n_val <- df$n[which.min(abs(df$effect - input$ss_effect))]
    
    ggplot(df, aes(x = effect, y = n)) +
      geom_line(color = "#2C3E50", linewidth = 1.2) +
      geom_vline(data = benchmarks, aes(xintercept = effect), 
                 linetype = "dashed", color = "#95A5A6", alpha = 0.7) +
      ...
      labs(x = "Expected Effect Size", y = "Required Sample Size (log scale)",
           title = paste("Sample Size for", round(input$ss_power * 100), "% Power"),
           subtitle = paste(tools::toTitleCase(input$ss_type), "-", 
                            if (input$ss_design == "single") "Single Sample" else "Two Groups")) +
      theme_minimal(base_size = 14) +
      theme(panel.grid.minor = element_blank())
  })
  ...
  output$power_table <- renderDT({
    ...
    datatable(df, 
      ...
      formatStyle(columns = 2:ncol(df),
                  backgroundColor = styleInterval(c(0.5, 0.8), c('#FADBD8', '#FCF3CF', '#D5F5E3')))
  })
  ...
  output$power_heatmap <- renderPlot({
    ...
    ggplot(grid, aes(x = factor(n), y = factor(effect), fill = power)) +
      ...
      labs(x = "Sample Size", y = "Effect Size",
           title = paste("Power Heatmap -", tools::toTitleCase(type))) +
      theme_minimal(base_size = 14) +
  ...
  output$quick_ref_table <- renderUI({
    ...
    tagList(
      ...
      tags$table(class = "table table-sm table-striped",
        tags$thead(tags$tr(tags$th("Parameter"), tags$th("Small"), tags$th("Medium"), tags$th("Large"))),
        tags$tbody(
          tags$tr(tags$td("Amplitude"), tags$td(amp_diff_small), tags$td(amp_diff_med), tags$td(amp_diff_large))
        )
      )
    )
  })
